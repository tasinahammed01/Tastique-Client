...
const PopulerFoods = () => {
  ...
  useEffect(() => {
    setBookmarkedFoods(getBookmarks());
  }, []);
  ...
  // Note: saving is handled in toggleBookmark util to avoid overwriting on mount

  // Add/Remove bookmark
  const handleBookmark = (food) => {
    const updated = toggleBookmark(bookmarkedFoods, food);
    setBookmarkedFoods(updated);
        ...
        {populerFood.slice(0, 6).map((food, index) => (
          ...
          >
            <div className="relative h-[300px] object-cover">
              ...
              {/* Bookmark Icon */}
              <button
                onClick={() => handleBookmark(food)}
                className="absolute top-3 left-4 cursor-pointer text-white text-xl"
              >
                {bookmarkedFoods.find((item) => (item.id || item._id) === (food._id || food.id)) ? (
